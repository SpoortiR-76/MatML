...
export default function RepairCurveChart({ initialResult }) {
    ...
    async function load() {
      ...
      try {
        ...
        const next = responses.map((r, idx) => {
          const life = r?.degradation?.lifespan_index ?? 0
          return {
            cycle: idx,
            su: r?.repair?.su_post_mpa ?? r?.strength?.su_mpa ?? 0,
            sy: r?.repair?.sy_post_mpa ?? r?.strength?.sy_mpa ?? 0,
            lifespan_index: life,
  ...
  const suBaseline = initialResult?.strength?.su_mpa ?? series[0].su
  const syBaseline = initialResult?.strength?.sy_mpa ?? series[0].sy
  const maxRepairs = initialResult?.recommendation?.max_repair_cycles ?? 8
  ...
  const topShap = Object.entries(shapMap)
    ...
    .slice(0, 3)
    .map(([feature, value]) => ({ feature, contribution: Number(value) }))
      ...
      <div className="h-64 rounded-xl p-2" style={{ background: 'rgba(255,255,255,0.03)' }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={series}>
            ...
            <ReferenceLine x={maxRepairs} stroke="#ef4444" strokeDasharray="4 4" />
